...
bool GameMenuLayer::init()
{
	do 
	{
		...
		titleSprite->setPosition(ccp(winSize.width/2, winSize.height * 0.80));
		this->createMenu();
		return true;
	} while (0);
	return false;
...
void GameMenuLayer::createMenu(void)
{
	...
	startNormalTextSprite->setPosition(ccp(startNormalBgSize.width / 2, startNormalBgSize.height / 2 + 23));
	...
	startSelectedTextSprite->setPosition(ccp(startNormalBgSize.width / 2, startNormalBgSize.height / 2 + 23));
